...
for ss = 1:length(EEGv.dataEpoch)
    
epochs = EEGv.dataEpoch{ss};
...
nEp=length(epochs);
CHpxx={};
clear Mpxx PSDband PSDbandSets PSDbandLog PSDbandLogSets
for j=1:length(CH)
    ch=CH(j); 
...
EEGv.PSD.pxx{ss}=pxx;                       % PSD samp x epochs
% EEGv.PSD.pxxLog=pxxLog;                 % PSD samp x epochs
EEGv.PSD.CHpxx{ss}=CHpxx;                   % PSD samp x channel

EEGv.PSD.PSDband{ss}=PSDband;               % PSD band x channel
EEGv.PSD.PSDbandLog{ss}=PSDbandLog;         % PSD band x channel Log
EEGv.PSD.PSDbandSets{ss}=PSDbandSets;       % PSD band x EEG regions
EEGv.PSD.PSDbandLogSets{ss}=PSDbandLogSets; % PSD band x EEG regions Log
